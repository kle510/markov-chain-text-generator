...
public class RandomTextGenerator {
	...
	public void outputText(String outFile, int numWords, int prefixLength, boolean DEBUG) throws IOException {
		...
		if (DEBUG) {
			System.out.println("DEBUG: chose an initial prefix: " + currentWordSequence);
		}
		...
		for (int i = 0; i < numWords; i++) {
			if (DEBUG) {
				System.out.println("DEBUG: prefix: " + currentWordSequence);
			}
			...
			if (DEBUG) {
				System.out.println("DEBUG: successors: " + prefix.getAllSuccessors(currentWordSequence));
			}
			if (DEBUG) {
			...
			if (DEBUG) {
				System.out.println("DEBUG: Output so far: " + output);
			}
